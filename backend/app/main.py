...
async def chat_endpoint(request: Request):
    try:
        ...
        # Обновили системный промпт: теперь ИИ отвечает коротко, если не попросить о развернутом ответе
        system_prompt = (
            "Ты — полезный, вежливый и умный ИИ-помощник. Отвечай ВСЕГДА строго на русском языке. "
            "Твое важное правило: отвечай очень коротко, буквально в 2-3 предложения. "
            "Однако, ЕСЛИ пользователь прямо просит ответить подробно, развернуто или расписать детали, "
            "тогда давай полный и детальный ответ."
        )
