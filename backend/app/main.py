...
async def chat_endpoint(request: Request):
    try:
        ...
        return {"text": "Ошибка: некорректный запрос", "model_used": "none"}
    ...
    if mode == "private":
        system_prompt = (
            ...
            f"Отвечай коротко. Никакой лишней «воды» и длинных вступлений. Срочно помни прошлый контекст беседы! "
            f"Если пользователю понадобятся подробности, он сам попросит дать развернутый ответ. "
            f"Не используй лишние кавычки вокруг текста. Для оформления используй обычные абзацы. "
            f"Вместо жирного шрифта (звездочек) используй заглавные буквы для заголовков. "
            f"Используй эмодзи для красоты. Ссылки пиши просто текстом. "
            f"Пользователь сидит с телефона. Есть приложение Pydroid, Github, Render, Openrouter, GROQ."
        )
    else:
        system_prompt = (
            f"Ты — друг и помощник. Пользователя зовут {user_name}. Текущие дата и время: {client_time}. "
            f"ОТВЕЧАЙ КОРОТКО И ПО СУТИ, без длинных рассуждений. "
            f"Не используй лишние кавычки вокруг текста. Вместо жирного шрифта (звездочек) используй заглавные буквы. "
            f"Щедро используй эмодзи. Пользователь сидит с мобильного телефона."
        )
        ...
        if (reply.startswith("Ошибка") or user_image) and (requested_model in ["auto", "gemini"]) and gemini_key:
            try:
                ...
                for m in history_messages:
                    ...
                    gemini_history_text += f"{name}: {m['content']}\n"
...
async def get_history():
    ...
    try:
        ...
        logger.error(f"Ошибка get_history: {e}")
        return []
